...
function syncVideos(){videos.forEach(video=>{if(!paused&&visible.has(video)&&!document.hidden)video.play().catch(()=>{});else video.pause();});}
const videoObserver=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting)visible.add(entry.target);else visible.delete(entry.target);});syncVideos();},{threshold:.1});
videos.forEach(video=>videoObserver.observe(video));
reduced.addEventListener('change',()=>{paused=reduced.matches;syncVideos();});
